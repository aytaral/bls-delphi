...
type
  TfrmExceptionDialog = class(TForm)
    SendBtn: TButton;
    TextLabel: TMemo;
    OkBtn: TButton;
    DetailsBtn: TButton;
  ...
  TfrmExceptionDialogClass = class of TfrmExceptionDialog;

var
  frmExceptionDialogClass: TfrmExceptionDialogClass = TfrmExceptionDialog;
...
uses
  ClipBrd, Math,
  JclBase, JclFileUtils, JclHookExcept, JclPeImage, JclAnsiStrings,
...
var
  frmExceptionDialog: TfrmExceptionDialog;
...
begin
  if JclValidateModuleAddress(ExceptAddr)
    ...
    TfrmExceptionDialog.ExceptionHandler(nil, Exception(ExceptObject))
  else
    SysUtils.ShowException(ExceptObject, ExceptAddr);
end;
...
procedure TfrmExceptionDialog.AfterCreateDetails;
begin
...
procedure TfrmExceptionDialog.BeforeCreateDetails;
begin
...
function TfrmExceptionDialog.ReportMaxColumns: Integer;
begin
  Result := 78;
end;
...
procedure TfrmExceptionDialog.SendBtnClick(Sender: TObject);
begin
  with TJclEmail.Create do
  try
    ParentWnd := Application.Handle;
    Recipients.Add('support@fakuva.no');
    Subject := 'Bug Report';
    Body := ReportAsText;
    SaveTaskWindows;
    try
...
procedure TfrmExceptionDialog.CopyReportToClipboard;
begin
...
procedure TfrmExceptionDialog.CreateDetails;
begin
...
procedure TfrmExceptionDialog.CreateReport;
var
  SL: TStringList;
  I: Integer;
...
procedure TfrmExceptionDialog.DetailsBtnClick(Sender: TObject);
begin
...
class procedure TfrmExceptionDialog.ExceptionHandler(Sender: TObject; E: Exception);
begin
...
class procedure TfrmExceptionDialog.ExceptionThreadHandler(Thread: TJclDebugThread);
begin
  if ExceptionShowing then
  begin
...
procedure TfrmExceptionDialog.FormCreate(Sender: TObject);
begin
...
procedure TfrmExceptionDialog.FormDestroy(Sender: TObject);
begin
...
procedure TfrmExceptionDialog.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TfrmExceptionDialog.FormPaint(Sender: TObject);
begin
...
procedure TfrmExceptionDialog.FormResize(Sender: TObject);
begin
...
procedure TfrmExceptionDialog.FormShow(Sender: TObject);
begin
  BeforeCreateDetails;
...
function TfrmExceptionDialog.GetReportAsText: string;
begin
...
procedure TfrmExceptionDialog.NextDetailBlock;
begin
...
function TfrmExceptionDialog.ReportNewBlockDelimiterChar: Char;
begin
  Result := '-';
end;
...
procedure TfrmExceptionDialog.SetDetailsVisible(const Value: Boolean);
var
...
class procedure TfrmExceptionDialog.ShowException(E: TObject; Thread: TJclDebugThread);
begin
  if frmExceptionDialog = nil then
    frmExceptionDialog := TfrmExceptionDialogClass.Create(Application);
  try
    with frmExceptionDialog do
    begin
    ...
    FreeAndNil(frmExceptionDialog);
  end;
end;

//--------------------------------------------------------------------------------------------------

procedure TfrmExceptionDialog.UMCreateDetails(var Message: TMessage);
begin
  Update;
  CreateDetails;
end;
...
procedure TfrmExceptionDialog.UpdateTextLabelScrollbars;
begin
...
begin
  ...
  begin
    ...
    AppEvents.OnException := TfrmExceptionDialog.ExceptionHandler;
    ...
    JclDebugThreadList.OnSyncException := TfrmExceptionDialog.ExceptionThreadHandler;
