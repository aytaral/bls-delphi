...
begin
  ...
  try
    ...
    Rpt := StringReplace(Rpt, ' ', '_', [rfReplaceAll]);
    if Rpt = '' then
      Rpt := 'Rapport';

    EMail.Recipients.Add('oppgi_epost@domene.no');
    Email.Subject := Rpt;
  ...
  if SaveDialog.Execute then begin
    ...
    if Ext = '' then begin
      case SaveDialog.FilterIndex of
        ...
        2: ChangeFileExt(SaveDialog.FileName, '.rft');
...
begin
  ...
  frPreview.SetFocus;
  acZoom100.Execute;
  Caption := Caption + ' - ' + frPreview.Report.ReportOptions.Name;
end;
